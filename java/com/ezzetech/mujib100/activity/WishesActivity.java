...
import com.ezzetech.mujib100.apiAdapter.WishesAdapter;
...
public class WishesActivity extends AppCompatActivity implements OnNetworkStateChangeListener {

    MujibApp mujibApp = new MujibApp();
    private RecyclerView wishesRV;
    private TextView noInternetTVED;

    List<Datum> wishList = new ArrayList<>();
    ...
    private void loadDataFromAPI() {
        ...
        apiService.getwishesResponse().enqueue(new Callback<WishesResponse>() {
            @Override
            public void onResponse(Call<WishesResponse> call, Response<WishesResponse> response) {
                if (response.isSuccessful()) {
                    WishesResponse wishesResponse = response.body();
                    wishList = wishesResponse.getData();
                    loadWishes();
                }
            }

            @Override
            public void onFailure(Call<WishesResponse> call, Throwable t) {
    ...
    private void init() {
        wishesRV = findViewById(R.id.wishesRV);
    ...
    private void loadWishes() {
        wishesAdapter = new WishesAdapter(WishesActivity.this, wishList);
        wishesRV.setLayoutManager(new LinearLayoutManager(this));
        wishesRV.setAdapter(wishesAdapter);
        swipeRefreshLayout.setRefreshing(false);
