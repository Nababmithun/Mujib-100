...
import com.ezzetech.mujib100.apiAdapter.PhotoArchiveAdapter;
...
public class PhotoArchiveActivity extends AppCompatActivity implements OnNetworkStateChangeListener {

    MujibApp mujibApp = new MujibApp();

    private RecyclerView photoArchiveRV;
    TextView noInternetTVED;

    private PhotoArchiveAdapter photoArchiveAdapter;
    private List<Datum> photoArchiveList = new ArrayList<>();
    ...
    private void loadDataFromAPI() {
        ...
        apiService.getPhotoArchiveResponse().enqueue(new Callback<PhotoArchiveResponse>() {
            @Override
            public void onResponse(Call<PhotoArchiveResponse> call, Response<PhotoArchiveResponse> response) {

                if (response.isSuccessful()){
                    PhotoArchiveResponse archiveResponse = response.body();
                    photoArchiveList =  archiveResponse.getData();
                    loadPhotoArchive();
                }
            ...
            @Override
            public void onFailure(Call<PhotoArchiveResponse> call, Throwable t) {
                Toast.makeText(PhotoArchiveActivity.this, ""+t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }

    private void loadPhotoArchive() {
        photoArchiveAdapter = new PhotoArchiveAdapter(this,photoArchiveList);
        photoArchiveRV.setLayoutManager(new LinearLayoutManager(this));
        photoArchiveRV.setAdapter(photoArchiveAdapter);
        swipeRefreshLayout.setRefreshing(false);
        photoArchiveAdapter.notifyDataSetChanged();
    }
    ...
    private void init() {
        photoArchiveRV = findViewById(R.id.photoArchiveRV);
