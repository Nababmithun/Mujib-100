...
import com.ezzetech.mujib100.apiAdapter.RecognizeAdapter;
...
public class RecognitionsActivity extends AppCompatActivity implements OnNetworkStateChangeListener {

    MujibApp mujibApp = new MujibApp();

    private RecyclerView recognizeRV;
    TextView noInternetTVED;
    RecognizeAdapter recognizeAdapter;

    List<Datum> recognizeList = new ArrayList<>();
    ...
    private void loadDataFromAPI() {
        ...
        apiService.getRecognitionResponse().enqueue(new Callback<RecognitionResponse>() {
            @Override
            public void onResponse(Call<RecognitionResponse> call, Response<RecognitionResponse> response) {
                if (response.isSuccessful()) {
                    ...
                    recognizeList = recognitionResponse.getData();
                    loadRecognize();
                }
            }
            ...
            public void onFailure(Call<RecognitionResponse> call, Throwable t) {
                ...
                Toast.makeText(RecognitionsActivity.this, "" + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void init() {
        recognizeRV = findViewById(R.id.recognizeRV);
    ...
    private void loadRecognize() {
        recognizeAdapter = new RecognizeAdapter(this, recognizeList);

        LinearLayoutManager layoutManager
                = new LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false);

        recognizeRV.setLayoutManager(layoutManager);
        recognizeRV.setAdapter(recognizeAdapter);
        swipeRefreshLayout.setRefreshing(false);
