...
import com.ezzetech.mujib100.apiAdapter.DocumentariesAdapter;
...
public class DocumentariesActivity extends AppCompatActivity implements OnNetworkStateChangeListener {
    ...
    List<Datum> documentariesList = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_documentaries);
    ...
    private void loadDataFromAPI() {
        ...
        apiService.getDocumentariesResponse().enqueue(new Callback<DocumentariesResponse>() {
            @Override
            public void onResponse(Call<DocumentariesResponse> call, Response<DocumentariesResponse> response) {
                if (response.isSuccessful()) {
                    DocumentariesResponse documentariesResponse = response.body();
                    documentariesList = documentariesResponse.getData();
                    loadDocumentaries();
                }
            }

            @Override
            public void onFailure(Call<DocumentariesResponse> call, Throwable t) {
    ...
    private void init() {
        documentariesRV = findViewById(R.id.documentariesRV);
    ...
    private void loadDocumentaries() {
        documentariesAdapter = new DocumentariesAdapter(this, documentariesList);
        documentariesRV.setLayoutManager(new LinearLayoutManager(this));
        documentariesRV.setAdapter(documentariesAdapter);
        swipeRefreshLayout.setRefreshing(false);
