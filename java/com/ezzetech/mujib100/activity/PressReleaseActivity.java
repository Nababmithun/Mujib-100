...
import com.ezzetech.mujib100.apiAdapter.PressReleaseAdapter;
...
public class PressReleaseActivity extends AppCompatActivity implements OnNetworkStateChangeListener {

    MujibApp mujibApp = new MujibApp();

    private RecyclerView pressReleaseRV;
    private TextView noInternetTVED;

    private PressReleaseAdapter pressReleaseAdapter;
    List<Datum> pressReleaseList = new ArrayList<>();
    ...
    private void loadDataFromAPI() {
        ...
        apiService.getPressReleaseResponse().enqueue(new Callback<PressReleaseResponse>() {
            @Override
            public void onResponse(Call<PressReleaseResponse> call, Response<PressReleaseResponse> response) {
                if (response.isSuccessful()) {
                    PressReleaseResponse releaseResponse = response.body();
                    pressReleaseList = releaseResponse.getData();
                    loadPressRelease();
                }
            }

            @Override
            public void onFailure(Call<PressReleaseResponse> call, Throwable t) {
    ...
    private void loadPressRelease() {
        pressReleaseAdapter = new PressReleaseAdapter(this, pressReleaseList);
        pressReleaseRV.setLayoutManager(new LinearLayoutManager(this));
        pressReleaseRV.setAdapter(pressReleaseAdapter);
        swipeRefreshLayout.setRefreshing(false);
        pressReleaseAdapter.notifyDataSetChanged();
    }

    private void init() {
        pressReleaseRV = findViewById(R.id.pressReleaseRV);
